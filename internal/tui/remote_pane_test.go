...
func TestRemotePaneResizeUpdatesEmulatorImmediately(t *testing.T) {
	...
	// Resize is synchronous (channel-based approach: emulator owned by main goroutine).
	if rp.Emulator().Width() != 120 || rp.Emulator().Height() != 40 {
		t.Errorf("emu dimensions = %dx%d, want 120x40",
...
func TestRemotePaneSendKey_PlainEnter(t *testing.T) {
	server, client := net.Pipe()
	defer client.Close()
	...
	// Plain Enter should produce \r (submit).
	...
	case data := <-done:
		if string(data) != "\r" {
			t.Errorf("SendKey(Enter) produced %q, want %q", string(data), "\r")
		}
	case <-time.After(time.Second):
		t.Fatal("timeout waiting for SendKey data")
	}
