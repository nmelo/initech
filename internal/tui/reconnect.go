...
type peerManager struct {
	...
	onAgentStatus func(name string, beads []string, primary, desc string, ws WaitingState, suspended bool)
		...
		case ev := <-pc.mux.Events():
			...
			case agentStatusAction:
				if pm.onAgentStatus != nil {
					pm.onAgentStatus(ev.Name, ev.Beads, ev.Bead, ev.Text, ev.WaitingState, ev.Suspended)
				}
...
// (ini-35ak).
func (pm *peerManager) SetOnAgentStatus(fn func(name string, beads []string, primary, desc string, ws WaitingState, suspended bool)) {
	pm.onAgentStatus = fn
}
