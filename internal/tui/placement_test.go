...
func planNames(t *testing.T, tui *TUI, connected map[string]bool) []string {
	...
	for _, p := range tui.panes {
		if rendersInWindow(paneKey(p), tui.windowID, tui.assignment, tui.layoutState.GroupOf, connected) {
	...
	for _, order := range []string{"hide-then-move", "move-then-hide"} {
		t.Run(order, func(t *testing.T) {
			...
			for _, p := range w2.panes {
				if rendersInWindow(paneKey(p), w2.windowID, a, w2.layoutState.GroupOf, conn) &&
					!w2.layoutState.Hidden[paneKey(p)] {
					visible2++
				}
			}
			if visible2 != 1 {
...
// TestFleetSurfaces_StayWholeFleet pins grooming AC 1, the no-overcorrect
// guard: the agents MODAL shows the whole fleet from every window -- only
// pane rendering filters by assignment. A fix that filtered the modal's
// members would break a decided behavior to fix this bug.
func TestFleetSurfaces_StayWholeFleet(t *testing.T) {
	_, w2, _ := placementTUIs(t, "group_window:\n    eng: window-2\n")

	members := w2.agentsGroupMembers()
	total := 0
