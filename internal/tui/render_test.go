...
func TestApplyLayout_ReservesStatusBar(t *testing.T) {
	...
	// The single pane should fill height 38 (40 - 2 for status bar area).
	if len(tui.plan.Panes) != 1 {
		t.Fatalf("expected 1 pane in plan, got %d", len(tui.plan.Panes))
	}
	pr := tui.plan.Panes[0]
	if pr.Region.H != 38 {
		t.Errorf("pane height = %d, want 38 (screen 40 - 2 status bar)", pr.Region.H)
	}
	// Pane should not extend into the status bar area (rows 38-39).
